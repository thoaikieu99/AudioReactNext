...
const singtoken = id => {
    return jwt.sign({ id }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPRIES_IN });
}
...
const login = catchAsync(async (req, res, next) => {
    ...
    if (!findus || !await findus.validatePassword(password_hash)) {
        ...
        createSendTocken(findus, 201, res);
    };
...
const protect = catchAsync(async (req, res, next) => {
    ...
    if (!freshUser) {
        return next(new AppError('The user belonging to this token does no longer exits.'))
    }
...
const forgotPassword = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        await user.save({ skip: ['confirmedPassword', 'password_hash'] });
        return next(new AppError('There are an error sending the email. Try again later!', 404));
    }

})
const resetPassword = catchAsync(async (req, res, next) => {
    let hasdtoken = crypto.createHash('sha256').update(req.params.token).digest('hex');
